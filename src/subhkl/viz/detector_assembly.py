...
def plot_unrolled_detector(
    ...
    for img_key, img in images.items():
        ...
        mesh = ax.pcolormesh(
            roty, Y, img, shading="auto", cmap="binary", norm=global_norm
        )
    ...
    if (
        ...
        is_isotropic = np.allclose(peaks.var_u, peaks.var_v) and np.allclose(
            peaks.cov_uv, 0
        )
    ...
    if (
        ...
        and getattr(peaks, "var_u", None) is None
    ):
        p_rotys = []
        p_Ys = []
