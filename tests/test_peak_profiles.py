...
def generate_synthetic_data():
    ...
    for run_id in [0, 1]:
        ...
        for i in range(num_peaks):
            ...
            gaussian = 100.0 * np.exp(-0.5 * ((yy - r_c)**2 / var_r + (xx - c_c)**2 / var_c))
            img += gaussian
...
def test_rbf_integrator_goniometer_kinematics():
    ...
    assert np.all(intensities > 0), "Found negative intensities, the unconstrained OLS solver failed."
    ...
    assert np.mean(var_u_0) > np.mean(var_v_0), "Run 0 did not recover horizontal major axis."
    ...
    assert np.mean(var_v_1) > np.mean(var_u_1), "Run 1 did not correctly project the rotated 3D tensor."
